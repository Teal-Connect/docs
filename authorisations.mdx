...
- **one_time** - Single authorisation for a specific action (e.g., one-time income verification). One-time authorisations expire after 1 day; the user must re-authorise if more data needs to be fetched.
...
<Note>
One-time authorisations automatically expire. After expiry, new data cannot be fetched, but existing data can still be read unless the authorisation is revoked or the term is deactivated.
</Note>
