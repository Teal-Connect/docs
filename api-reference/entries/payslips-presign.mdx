...
openapi: 'POST /entries/payslips/presign'
---
...
`POST /entries/payslips` to submit the uploaded documents for processing.
...
const response = await axios.post(
  'https://api.sandbox.goteal.co/entries/payslips/presign',
  {
    files: [
      { file_name: 'payslip1.pdf', external_id: 'ext-jan-2024' },
      { file_name: 'payslip2.pdf' }
...
$curl = curl_init('https://api.sandbox.goteal.co/entries/payslips/presign');
...
HttpResponse<String> response = Unirest.post("https://api.sandbox.goteal.co/entries/payslips/presign")
    .header("Authorization", "Bearer " + token)
    .header("Content-Type", "application/json")
    .body("{\n  \"files\": [\n    {\"file_name\": \"payslip1.pdf\", \"external_id\": \"ext-jan-2024\"},\n    {\"file_name\": \"payslip2.pdf\"}\n  ]\n}")
    .asString();
