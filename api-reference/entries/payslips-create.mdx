---
title: 'Submit presigned payslips'
openapi: 'POST /entries/payslips'
---

Use this endpoint after uploading payslip PDFs via `POST /entries/payslips/presign`.
Each object in the request body references a document uploaded in the previous
step. This replaces the legacy multipart upload flow.
...
// Step 1: request presigned URLs
export async function submitPresignedPayslips(files) {
  ...
  await Promise.all(
    presignData.presigned_urls.map(async (urlInfo, index) => {
      const file = files[index]
      await axios.put(urlInfo.presigned_url, file.contents, {
        headers: { 'Content-Type': 'application/pdf' },
      })
    })
  )
  ...
  const { data: submitData } = await axios.post(
    ...
    {
      files: presignData.presigned_urls.map((urlInfo, index) => ({
        path: urlInfo.path,
        external_document_id: files[index]?.externalId,
      })),
    },
    {
      headers: {
