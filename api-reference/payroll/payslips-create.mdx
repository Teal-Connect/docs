...
2. **Upload PDFs** – for every returned `presigned_url`, perform an HTTP `PUT`
   with `Content-Type: application/octet-stream`, streaming the PDF bytes.
   Searchable, machine-readable PDFs are preferred. Scanned-image PDFs go through
   OCR; if Doc Scan assesses trust as Medium or Low before OCR, the file is
   rejected and listed in `payslip_errors`.
3. **Submit metadata** – call `POST /payroll/entries/payslips` (this endpoint) with a
   ...
   the response. Successful submissions include `trust_score`. Retrieve
   `trust_analysis` and `pdf_metadata` from [GET /documents/{document_id}](/api-reference/documents/get).
   See [Doc Scan](/doc-scan).
