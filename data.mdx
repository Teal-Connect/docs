...
## 2. Consent-Protected Endpoints

Certain endpoints that create or retrieve payroll data on behalf of a user require a valid consent record before they can be accessed. This applies to:
...
If no active consent exists for the user, these endpoints will reject the request. You can optionally include the `x-consent-id` header to specify which consent to use; otherwise the system will resolve a valid consent automatically. See [Consents](/consents) for full details on creating terms and recording user consent.
