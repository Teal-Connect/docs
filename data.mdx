...
For a step-by-step, runnable walkthrough of these endpoints, see the [Sandbox guide](/sandbox).
...
Represents the borrowers and end-users that give authorised access and upload their payslips to our API.
...
A user can have multiple accounts to the same provider depending on the type of connection. The `POST /accounts` endpoint is a way to initialize a connection to a provider for a given user; Teal offers a separate provider flow for users via the WebUI as documented in [Introduction](/introduction). 
Alternatively, `POST /accounts/direct-connections` can be used to establish a connection directly by providing a user ID.
...
- Use the optional `extra_login_params` object when a provider requires additional key/value pairs; otherwise omit it.
#### Providers that require extra login parameters:
  - Dayforce: <span style={{ color: '#f29fb3' }}>Required</span> `extra_login_params.company_name`
...
```bash
curl --request POST \
  --url https://api.sandbox.goteal.co/payroll/entries/connections \
...
For an account, a call to the `/payroll/entries/connections` endpoint will query the connection for data. This endpoint is for adhoc data retrieval for an account; Teal will also collect the payroll data for users via the WebUI as soon
as a valid connection is established. This endpoint can be applied to any account, it does not matter if it was established via your own call to `POST /accounts` or via the Teal WebUI. Note that any new data retrieved via `/payroll/entries/connections` is persisted on Teal's storage and can be retrieved using the `/payroll/{user_id}` endpoint. 
...
A manual upload of payslip data using is not linked to an account, but the JSON response of `/payroll/entries/payslips` will be in the same format and the payroll data
...
For payslip uploads, the same payload includes `trust_score`, `trust_analysis`, and `pdf_metadata`. Bank statement documents leave those fields null. See [Doc Scan](/doc-scan) and [List documents](/api-reference/documents/list).
...
## 2. Authorisation-Protected Endpoints

Certain endpoints that create or retrieve payroll data on behalf of a user require a valid authorisation record before they can be accessed. This applies to:

- `POST /accounts` – creating a new account
- `PUT /accounts/{account_id}/mfa` – submitting mfa for an account
- `POST /accounts/direct-connections` – establishing a direct connection
- `POST /payroll/entries/connections` – querying a connection for data
- `POST /payroll/entries/payslips/presign` – generating presigned upload URLs
- `POST /payroll/entries/payslips` – submitting payslips for processing
- `PUT /documents/bank-statements/presign` – generating presigned bank statement upload URLs
- `PUT /documents/bank-statements` – submitting bank statements for processing

If no active authorisation exists for the user, these endpoints will reject the request. You can optionally include the `x-teal-authorisation-id` header to specify which authorisation to use; otherwise the system will resolve a valid authorisation automatically. See [Authorisations](/authorisations) for full details on creating terms and recording user authorisation.
