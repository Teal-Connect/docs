...
A user can have multiple accounts to the same provider depending on the type of connection. The status of a connection determines if it is able to recieve data, needs authorisation or if it is invalid or expired. An account
connected via OAUTH2 will not require credentials to be passed into the account creation request. Any credentials that are passed to Teal are encrypted securely.
...
For an account, a call to the `/entries/connections` endpoint will query the connection for data. This endpoint is for additonal adhoc data retrieval for an account; Teal will also collect the payroll data via the user WebFlow as soon
as a valid connection is established. Note that any new data retrieved via `/entries/connections` is persisted on Teal's storage and can be retrieved using the `/payroll/{user_id}` endpoint. 

A manual upload of payslip data using is not linked to an account, but the JSON response of `/entries/payslip` will be in the same format to that of the `/entries/connections` endpoint and the payroll data
can be retireved in the exact same way. The files uploaded must be in PDF format.
...
A deletion of a user deletes all sub-resources that the user is linked to. This includes accounts, payroll and uploaded documents.
