...
<Note>
HMRC connections require fraud prevention headers. See the [Fraud Headers](#fraud-headers) section below for details if using the `hmrc` endpoints. This is already handled for Teal WebApp usage.
</Note>
...
HMRC requires fraud prevention headers on API requests to help protect users' confidential data from criminals and fraudsters. This data supports HMRC's transaction [monitoring](https://developer.service.hmrc.gov.uk/guides/fraud-prevention/#why-you-must-send-data). 
...
| `x-teal-client-timezone` | `UTC±hh:mm` | `UTC+00:00` |
