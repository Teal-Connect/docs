...
To get the appropriate consent term for a user based on their recurring check configuration:

```bash
curl --request GET \
  --url 'https://api.sandbox.goteal.co/consents/terms/latest?user_id=95a0e70b-fe02-4f47-aef9-2efff279df71' \
  --header 'X-API-KEY: <api-key>'
```
...
| `ip_address` | Optional. If not provided, captured from request headers. Use when recording consent via server-to-server calls. |
...
- **is_valid** - Whether consent is currently valid (status is active AND term is active)
...
Each consent record includes an `is_valid` field that indicates whether the consent is currently valid:

- `is_valid: true` - Consent status is `active` AND the associated term is active
- `is_valid: false` - Either consent is revoked OR the term has been deactivated

If `is_valid` is false because the term was deactivated, the user needs to re-consent to the new term.
