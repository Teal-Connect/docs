...
- The user has an active recurring authorisation

<Warning>
Users without an active recurring authorisation will have their accounts ignored during recurring checks, even if all other eligibility criteria are met. Ensure that each user has accepted a valid `recurring` authorisation term before relying on recurring payroll updates. See [Authorisations](/authorisations) for details on recording and managing authorisations.
</Warning>
