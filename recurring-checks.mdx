...
- The user has an active recurring consent

<Warning>
Users without an active recurring consent will have their accounts ignored during recurring checks, even if all other eligibility criteria are met. Ensure that each user has accepted a valid `recurring` consent term before relying on recurring payroll updates. See [Consents](/consents) for details on recording and managing consent.
</Warning>
