...
Set the iframe `src` to the `webapp_link`. The redirect will set a session cookie and load the webapp inside the frame. For clients that configure a `client_callback_url`, the webapp communicates journey outcomes back to the parent page via `postMessage` (type: `teal:journey_outcome`).

If cookie-setting is blocked by the browser (e.g. Safari ITP), the webapp requests the token from the parent via a `teal:session_required` message. The parent should respond with `{ type: "teal:session_token", token: "<jwt>" }` to complete the session exchange.
