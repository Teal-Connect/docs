...
For certain providers, you can test realistic flows by sending specific usernames that match predefined scenarios. These scenarios simulate real-world conditions such as MFA challenges, incorrect credentials, and successful logins.
...
| Provider | Test User | Scenario | Username | Payroll Date Range | MFA | MFA Type | MFA Code |
|---|---|---|---|---|---|---|---|
| BrightPayCloud | Sarah Mitchell | happy_no_mfa | `happy_no_mfa` | 2025-08-01 to 2025-08-31 | No | — | — |
| CGI | David Patel | happy_mfa | `happy_mfa` | 2025-10-01 to 2025-10-31 | Yes | PasswordWithPositions — Pin Code (positions 2, 4, 6) | `123` |
| AdpMyAdp | Emma Richardson | happy_no_mfa | `happy_no_mfa` | 2025-05-01 to 2025-05-31 | No | — | — |
| Hero | Marcus Thompson | happy_mfa | `happy_mfa` | 2025-05-01 to 2025-05-31 | Yes | Code — Authenticator App | `140295` |

The available scenario usernames are: `happy_mfa`, `happy_no_mfa`, `wrong_mfa`, `wrong_username`, and `wrong_password`.

These scenarios can also be used on the [sandbox webapp](https://app.sandbox.goteal.co/). When testing via the webapp, ensure the user's `payroll_period_months` is set large enough to cover the scenario's payroll date range.
